          ...
          path.traverse({
            ...
            SwitchStatement(path2) {
              ...
              for (i = 0; i < path2.node.cases.length - 1; i++) {
                ...
                if (cases_list[i].consequent.length) {
                  if (
                    cases_list[i].consequent[0].type == 'VariableDeclaration'
                  ) {
                    //遇到new对象处理
                    if (
                      cases_list[i].consequent[0].declarations[0].init.type ==
                      'NewExpression'
                    ) {
                      //获得type
                      node_code =
                        cases_list[i].consequent[0].declarations[0].init;
                      ...
                      try {
                        id_name =
                          cases_list[i].consequent[1].expression.expressions[1]
                            .callee.property.name;
                      } catch (e) {
                        id_name =
                          cases_list[i].consequent[2].expression.callee.property
                            .name;
                      }
                      // console.log(id_name);
                    ...
                    //其他情况处理
                      ...
                      if (
                        cases_list[i].consequent[0].declarations[0].init.callee
                      )
                        id_type =
                          cases_list[i].consequent[0].declarations[0].init
                            .callee.property.name;
                      if (
                        ...
                        id_type =
                          cases_list[i].consequent[0].declarations[0].init
                            .consequent.callee.property.name;
                      id_type = get_id_type(id_type);
                      // console.log(id_type);
                      if (cases_list[i].consequent[1].type == 'ForStatement')
                        id_name =
                          cases_list[i].consequent[1].body.body[0].expression
                            .callee.property.name;
                      else if (
                        cases_list[i].consequent[1].type ==
                        'ExpressionStatement'
                      ) {
                        if (cases_list[i].consequent[1].expression.expressions)
                          id_name =
                            cases_list[i].consequent[1].expression.expressions[
                              cases_list[i].consequent[1].expression.expressions
                                .length - 1
                            ].callee.property.name;
                        else
                          id_name =
                            cases_list[i].consequent[1].expression.callee
                              .property.name;
                      }
                  ...
                  if (
                    cases_list[i].consequent[0].type == 'ExpressionStatement'
                  ) {
                    //一般处理
                    if (
                      cases_list[i].consequent[0].expression.right.type ==
                      'CallExpression'
                    ) {
                      id_type =
                        cases_list[i].consequent[0].expression.right.callee
                          .property.name;
                      // console.log(id_type)
                      id_type = get_id_type(id_type);
                      // console.log(id_type);
            ...
            IfStatement(path3) {
              ...
              if (path3.node.consequent.type === 'BlockStatement') {
                if (
                  path3.node.consequent.body[0].type === 'VariableDeclaration'
                ) {
                  // console.log(generator(path3.node).code)
                  //遇到new对象处理
                  if (
                    path3.node.consequent.body[0].declarations[0].init.type ==
                    'NewExpression'
                  ) {
                    //获得type
                    node_code =
                      path3.node.consequent.body[0].declarations[0].init;
                    // console.log(generator(node_code).code)
                    node_code_list = generator(node_code).code.split('.');
                    node_code_list.shift();
                    id_type = node_code_list.join('_').replace('()', '');
                    ...
                    try {
                      id_name =
                        path3.node.consequent.body[0].expression.callee.property
                          .name;
                    } catch (e) {
                      id_name =
                        path3.node.consequent.body[1].expression.expressions[1]
                          .callee.property.name;
                    }
                    ...
                    id_type =
                      path3.node.consequent.body[0].declarations[0].init.callee
                        .property.name;
                    id_type = get_id_type(id_type);
                    id_name =
                      path3.node.consequent.body[1].expression.callee.property
                        .name;
                    if (id_name.startsWith('set')) {
                      id_st = '';
                    } else if (id_name.startsWith('add')) {
              ...
              if (path3.node.consequent.type === 'ForStatement') {
                id_type =
                  path3.node.consequent.init.declarations[0].init.consequent
                    .callee.property.name;
                id_type = get_id_type(id_type);
                id_name =
                  path3.node.consequent.body.expression.callee.property.name;
                // console.log(id_name);
                if (id_name.startsWith('set')) {
                  id_st = '';
                } else if (id_name.startsWith('add')) {
...
const visitor2 = {
  AssignmentExpression(path) {
    if (
      ...
      path.node.left.type === 'MemberExpression' &&
      path.node.right.properties.length
    ) {
      left_node = path.node.left;
